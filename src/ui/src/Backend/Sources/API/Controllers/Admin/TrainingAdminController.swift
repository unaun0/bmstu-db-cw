...
public final class TrainingAdminController: RouteCollection {
    ...
    public func boot(routes: RoutesBuilder) throws {
        ...
        trainingRoutes.grouped(
            ...
            "specialization",
            ":specialization-id",
            use: getTrainingBySpecializationId
        ).openAPI(
            tags: .init(name: "Admin - Training"),
            summary: "Получить тренировки по ID специализации для администратора",
            description:
                "Возвращает тренировки по UUID специализации. Требует прав администратора.",
            response: .type([TrainingDTO].self),
            auth: .bearer()
        )
        trainingRoutes.grouped(
...
extension TrainingAdminController {
    ...
    @Sendable
    func getTrainingBySpecializationId(req: Request)
    async throws -> Response {
        return try await trainingService.find(
