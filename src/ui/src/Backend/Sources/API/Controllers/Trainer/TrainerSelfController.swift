...
public final class TrainerSelfController: RouteCollection {
    ...
    public func boot(routes: RoutesBuilder) throws {
        ...
        routes.grouped(
            ...
            use: getProfileSpecializations
        ).openAPI(
            tags: .init(name: "Trainer - Profile"),
            summary: "Получить специализации профиля тренера",
            description:
                "Возвращает специализации профиля тренера. Требуются права тренера.",
...
extension TrainerSelfController {
    ...
    @Sendable
    func getProfileSpecializations(req: Request) async throws -> Response {
        try await service.getProfileSpecializations(
            userId: req.auth.require(User.self).id
        ).map {
