...
public final class TrainerUserController: RouteCollection {
    ...
    public func boot(routes: RoutesBuilder) throws {
        ...
        trainerRoutes.grouped(uuidMiddleware).get(
            ...
            description: "Возвращает данные тренера по UUID пользователя.",
            response: .type(TrainerInfoDTO.self),
            auth: .bearer()
        )
    }
...
extension TrainerUserController {
    ...
    func getTrainerByUserId(req: Request) async throws -> Response {
        let userId = try req.parameters.require("user-id", as: UUID.self)
        let (trainer, user) = try await service.findTrainer(byUserID: userId)
