...
public final class TrainerReviewMongoDBRepository {
...
extension TrainerReviewMongoDBRepository: ITrainerReviewRepository {
    
    public func create(_ review: TrainerReview) async throws {
        try await TrainerReviewMongoDBDTO(from: review).create(on: db)
    }

    public func update(_ review: TrainerReview) async throws {
        guard
            let existing = try await TrainerReviewMongoDBDTO.find(review.id, on: db)
        else {
    ...
    public func delete(id: UUID) async throws {
        guard
            let entity = try await TrainerReviewMongoDBDTO.find(id, on: db)
        else {
            throw TrainerReviewError.reviewNotFound
        }
    ...
    public func find(id: UUID) async throws -> TrainerReview? {
        try await TrainerReviewMongoDBDTO.find(id, on: db)?.toTrainerReview()
    }

    public func find(trainerId: UUID) async throws -> [TrainerReview] {
        try await TrainerReviewMongoDBDTO.query(on: db)
