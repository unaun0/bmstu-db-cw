...
extension MembershipRepository: IMembershipRepository {
    public func create(_ membership: Membership) async throws {
        try await MembershipDBDTO(
            from: membership
        ).create(on: db)
    }
    ...
    public func update(_ membership: Membership) async throws {
        guard
            let existing = try await MembershipDBDTO.find(
                membership.id,
                on: db
            )
        else {
    ...
    public func find(id: UUID) async throws -> Membership? {
        try await MembershipDBDTO.find(id, on: db)?.toMembership()
    }

    public func find(userId: UUID) async throws -> [Membership] {
        try await MembershipDBDTO.query(
            on: db
        ).filter(
            \.$userId == userId
    ...
    public func delete(id: UUID) async throws {
        guard
            let membership = try await MembershipDBDTO.find(
                id,
                on: db
            )
        else {
