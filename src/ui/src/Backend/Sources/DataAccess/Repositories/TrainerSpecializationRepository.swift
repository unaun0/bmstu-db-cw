...
public final class TrainerSpecializationRepository {
...
extension TrainerSpecializationRepository: ITrainerSpecializationRepository {
    public func create(_ trainerSpecialization: TrainerSpecialization) async throws {
        try await TrainerSpecializationDBDTO(
    ...
    public func update(_ trainerSpecialization: TrainerSpecialization) async throws {
        guard
            let existing = try await TrainerSpecializationDBDTO.find(
    ...
    public func delete(id: UUID) async throws {
        guard
            let entity = try await TrainerSpecializationDBDTO.find(
                id,
                on: db
            )
        else {
    ...
    public func find(id: UUID) async throws -> TrainerSpecialization? {
        try await TrainerSpecializationDBDTO.find(
            id,
            on: db
    ...
    public func find(trainerId: UUID) async throws -> [TrainerSpecialization] {
        try await TrainerSpecializationDBDTO.query(
            on: db
        ).filter(
            \.$trainerId == trainerId
